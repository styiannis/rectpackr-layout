...
function checkModuleSystemMatches(entries) {
  const mismatched = [
    ...
    ({ path, expected }) =>
      ...
      expected &&
      !path.includes('*') &&
      // Extension settles it where there is one -- the build emits .mjs/.cjs
      // and .d.mts/.d.cts exclusively. A plain .js (the UMD bundle) has none,
      // so it falls back the way Node does: the root manifest's "type", there
      // being no package.json between it and the file.
      (path.endsWith('.mjs') || path.endsWith('.mts')
        ? 'module'
        : path.endsWith('.cjs') || path.endsWith('.cts')
          ? 'commonjs'
          : pkgType) !== expected
  );
